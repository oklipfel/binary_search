    ...
    def go(left, right):
        ...
        if left == right:
            if xs[left] == x:
                ...
                if left == len(xs) - 1 and xs[left] > x:
                    return left + 1
                if xs[left] > x:
                    return mid
                if xs[left] < x:
...
def count_repeats(xs, x):
    ...
    0
    '''
    high = helper_function_l(xs, x)
    low = helper_function_r(xs, x)
    if xs == []:
        return 0
...
def argmin(f, lo, hi, epsilon=1e-3):
    ...
    we will spend a lot of time talking about different f functions that
    can be minimized and their applications.
